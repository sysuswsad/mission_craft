...
def get_unread_num(user_id):
    """获取未读通知读数量
    """
    update_notification_login(user_id)
    db = get_db()
    notifications = db.execute(
        'SELECT *'
        ' FROM Notification'
...
def create_notification_type_1(mission_id):
    """
    ...
    db.execute(
        ...
        ' VALUES (?, ?, ?, ?, ?)',
        (info[0][0], mission_id, message, now_time, n_type,)
    )
    db.commit()
