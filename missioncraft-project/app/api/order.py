...
def create_order():
    ...
    order = db.execute(
        'SELECT * FROM MissionOrder WHERE order_state != 2 AND mission_id = ? AND receiver_id = ?', (mission_id, g.user['idUser'],)
    ).fetchone()
    ...
    db.execute(
      'INSERT INTO MissionOrder (mission_id, receiver_id, receive_time, qq, wechat, phone, other_way) VALUES (?, ?, ?, ?, ?, ?, ?)',
        (mission_id, g.user['idUser'], datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), qq, wechat, phone, other_way)
    )
...
def confirm_order():
    ...
    if mission_id and order_id is None:
        order_info = db.execute(
            'SELECT mission_id, receiver_id, order_state, idMissionOrder FROM MissionOrder WHERE order_state != 2 AND mission_id = ?', (mission_id,)
        ).fetchone()
    # 问卷领取者通过order_id确认
    elif order_id and mission_id is None:
        order_info = db.execute(
            'SELECT mission_id, receiver_id, order_state, idMissionOrder FROM MissionOrder WHERE order_state != 2 AND idMissionOrder = ?', (order_id,)
        ).fetchone()
    else:
    ...
    db.execute(
        ...
        (1, 1, 1, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), order_info['idMissionOrder'],)
    )
    db.execute(
